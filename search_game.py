...
def main(
    game_name: str = typer.Argument(..., help="the name of the game"), 
    quick_link: bool = typer.Option(False, help="if you want the link to the lowest price returned"),
    price: int = typer.Option(None, help="if you want to return all the shops less than a set price"),
    store: str = typer.Option(None, help="returns if the shop specified has the lowest price for the game"),
    hush: bool = typer.Option(False, help="does not display all the shops, prices, and links if True")
    ):

    typer.echo(typer.style('Searching..', fg=typer.colors.BRIGHT_BLUE))
    game = Game()
    game = game.search(game_name)
    found:bool = game[0]
    game:Game = game[1]


    if quick_link and found:
        print(game.link_to_lowest_price)
    
    if price is not None and found:
        found_price = game.less_than_price(price)
        if len(found_price) !=0:
            prices = 'found for less than {}\n'.format(price)

            for i in game.less_than_price(price):
                prices += str(i) + '\n'
            print(prices)
        else:
            typer.echo(typer.style("Could not find the game less than {}\n".format(price), fg=typer.colors.RED))

    if store is not None and found:
        found_store = game.store(store)
        if found_store is not None:
            print('found at {}\n'.format(store))
            print(game.store(store))
            print('\n')
        else:
            typer.echo(typer.style("Store {} does not have the game\n".format(store), fg=typer.colors.RED))
    
    if found and not hush:
        print(game)
